...
function initializeColorModifier() {
    ...
    document.querySelectorAll('.adjustment-scale').forEach((scale) => {
        ...
        scale.addEventListener('click', function (e) {
            if (e.target.classList.contains('scale-indicator')) return;
    ...
    function updateScaleIndicators() {
        document.getElementById('lightness-indicator').style.left =
            ((adjustments.lightness + 100) / 200) * 100 + '%';

        document.getElementById('saturation-indicator').style.left =
            ((adjustments.saturation + 100) / 200) * 100 + '%';

        document.getElementById('hue-indicator').style.left =
            (adjustments.hue / 360) * 100 + '%';

        document.getElementById('opacity-indicator').style.left =
            adjustments.opacity + '%';
    }
    ...
    function updateModified() {
        ...
        const rgb = hexToRgb(modifiedHex);
        const modifiedColor =
            opacity === 1
                ? modifiedHex
    ...
    function updateColorCodes(type, hex, opacity) {
        ...
        let html = `
            <button class="modifier-code-btn" data-hex="${hex}" title="Click to copy">${hex}</button>
