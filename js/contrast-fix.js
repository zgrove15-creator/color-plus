function initializeContrastFix() {
  ...
  document.addEventListener('keydown', function(e) {
    if ((e.ctrlKey || e.metaKey) && e.key === 'Enter') {
      const activeTab = document.querySelector('.tab-btn.active').getAttribute('data-tool');
    ...
    if ((e.key === 'l' || e.key === 'L') && !isInputFocused(e.target)) {
      ...
      if (activeTab === 'contrast') {
        ...
        showShortcutFeedback(lockColor1.checked ? 'Text locked' : 'Text unlocked');
      }
    }
    ...
    if ((e.key === 'k' || e.key === 'K') && !isInputFocused(e.target)) {
      ...
      if (activeTab === 'contrast') {
        ...
        showShortcutFeedback(lockColor2.checked ? 'Background locked' : 'Background unlocked');
      }
    }
  });
  ...
  function updateWCAGBadges() {
    ...
    if (contrast >= 3) {
      html += '<span class="wcag-badge pass">✓ AA Large</span>';
    } else {
      html += '<span class="wcag-badge fail">✗ AA Large</span>';
    }

    if (contrast >= 4.5) {
      html += '<span class="wcag-badge pass">✓ AA</span>';
    } else {
      html += '<span class="wcag-badge fail">✗ AA</span>';
    }

    if (contrast >= 7) {
      html += '<span class="wcag-badge pass">✓ AAA</span>';
    } else {
      html += '<span class="wcag-badge fail">✗ AAA</span>';
    }
  ...
  function tryAdjustment(color1, color2, targetContrast, direction, amount) {
    ...
    if (direction === 'lighten-text') {
      ...
      const text1Lightness = getColorLightness(color1);
      const bg2Lightness = getColorLightness(color2);
      
      while (chroma.contrast(c1, c2) < targetContrast && attempts < 50) {
        if (text1Lightness < bg2Lightness) {
          c1 = lightenColorWithBoost(c1, amount);
          c2 = darkenColorWithBoost(c2, amount);
        } else {
          c1 = darkenColorWithBoost(c1, amount);
  ...
  function rankAndDeduplicate(solutions) {
    ...
    for (const sol of ranked) {
      const isDuplicate = deduped.some(existing => {
        const textLDiff = Math.abs(getColorLightness(sol.color1) - getColorLightness(existing.color1));
        const bgLDiff = Math.abs(getColorLightness(sol.color2) - getColorLightness(existing.color2));
        return textLDiff < 0.03 && bgLDiff < 0.03;
      });
      if (!isDuplicate) {
        deduped.push(sol);
      }
  ...
  getResultsBtn.addEventListener('click', function() {
    ...
    if (isColor2Locked && !isColor1Locked) {
      const textDirs = ['lighten-text', 'darken-text'];
      ...
      const bgDirs = ['lighten-bg', 'darken-bg'];
      ...
      const allDirs = ['lighten-text', 'darken-text', 'lighten-bg', 'darken-bg', 'balance-smart'];
      ...
      const extremeDirs = ['lighten-text', 'darken-text', 'lighten-bg', 'darken-bg'];
      ...
      html += `
        <div class="result-card">
          ...
          <div class="result-hex-buttons">
            <button class="result-hex-button" data-hex="${solution.color1}" title="Text Color: Press C to copy">${solution.color1}</button>
            <button class="result-hex-button" data-hex="${solution.color2}" title="Background Color: Press B to copy">${solution.color2}</button>
          </div>
